...
from llm_graph_logic.providers.business_transformer import BusinessRequirements, TransformerBusinessInfoProvider
...
@pytest.fixture
def make_parser():
    def _make(project_path: str, requirements_paths: list[str]) -> Tuple[TerraformParser, List[BusinessRequirements]]:
        terraformParser = TerraformParser().addProvider(TerraformInfoProvider(project_path))
        requirements_list = []
        for requirement_path in requirements_paths:
            infoProvider = TransformerBusinessInfoProvider(Path(requirement_path))
...
def make_llm_processor(global_llm_proxy):
    def _make(cache: dc.Cache) -> LLMProcessor:
        return (
            BasePromptRegistry().updateRegistry(LLMProcessor())
...
def make_project(make_parser, make_llm_processor):
    def _make(project_id: str, project_path: str, requirements_paths: list[str], cache_path: str) -> Project:
        projectPathPath = Path(project_path)
...
def make_project_processor(make_project):
    def _make(project_id: str, project_path: str, requirements_paths: list[str], cache_path: str) -> ProjectsProcessor:
        project, requirements_list = make_project(project_id, project_path, requirements_paths, cache_path)
