...
async def test_basic(make_project_processor):
    ...
    project_path = f"tests/terraform/examples/bad_practices/security_risks/{project_id}"
    requirements_paths = ["tests/terraform/requirements/bad_practices.json"]
    cache_path = f"tests/terraform/cache/bad_practices/security_risks/{project_id}"

    processor = make_project_processor(project_id, project_path, requirements_paths, cache_path)
    await processor.processProjects()
    processor.saveGraph2VisJS(project_id, Path(f"tests/terraform/graphs/bad_practices/security_risks/test_{project_id}.json"))